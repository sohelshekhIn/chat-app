...
if (room == "" || name == "") {
  ...
  console.log("No User || gorup selected");
...
socket.on("message", function (message) {
  ...
  var $message = $('<li class = "list-group-item uMsg"></li>');

  var momentTimestamp = moment.utc(message.timestamp).local().format("h:mm A");
  //$(".messages").append($('<p>').text(message.text));
  // appending senders name in <p></p> tag
  $message.append("<p class='senderName'>" + message.name + "</p>");
  $message.append("<p class='reMsg'>" + message.text + "</p>");
  $message.append("<p class='time' >" + momentTimestamp + "</p>");
...
//
//
// fIt handles ant essages sent by me
//
...
$form.on("submit", function (event) {
  ...
  var $messages = $(".messages");
  var $message = $('<li class = "list-group-item miMsg"></li>');
